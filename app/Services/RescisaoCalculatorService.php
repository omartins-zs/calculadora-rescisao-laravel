    ...
    public function calcular(array $input): array
    {
        ...
        if ($tipoAviso === 'indenizado') {
            if ($motivo === 'dispensa_sem_justa_causa' || in_array($motivo, ['rescisao_antecipada_experiencia_empregador'])) {
                $valorAviso = round(($baseRemuneracao / 30) * $diasAviso, 2);
                ...
                $valorAviso = round((($baseRemuneracao / 30) * $diasAviso) / 2, 2);
                ...
                $avisoDescontoEmpregado = round(($baseRemuneracao / 30) * 30, 2); // Desconta 30 dias se não cumpriu
        ...
        if ($diasTrabalhados > 0) {
            $valorSaldoSalario = round(($baseRemuneracao / 30) * $diasTrabalhados, 2);
        ...
        if (($input['possui_ferias_vencidas'] ?? 'nao') === 'sim') {
            ...
            $valorFeriasVencidas = round(($baseRemuneracao * $qtd) + (($baseRemuneracao * $qtd) / 3), 2);
        ...
        if ($motivo !== 'dispensa_com_justa_causa') {
            ...
            if ($avosFerias > 0) {
                $valorFeriasProp = round(($baseRemuneracao / 12) * $avosFerias, 2);
                $terco = round($valorFeriasProp / 3, 2);
        ...
        if ($motivo !== 'dispensa_com_justa_causa') {
            ...
            if ($avos13 > 0) {
                $valor13 = round(($baseRemuneracao / 12) * $avos13, 2);
        ...
        if ($valorSaldoSalario > 0) {
            ...
            $inssSalario = round($resInssSalario['desconto'], 2);
            if ($inssSalario > 0) {
            ...
            $impostoSalario = round($resIrSalario['desconto'], 2);
            if ($impostoSalario > 0) {
                $items[] = ['categoria' => 'Descontos', 'nome' => 'IRRF (sobre Saldo Salário)', 'referencia' => '-', 'valor' => $impostoSalario, 'isento' => false];
                $totalDescontos += $impostoSalario;
            }
        }
        ...
        if ($valor13 > 0) {
            ...
            $inss13 = round($resInss13['desconto'], 2);
            if ($inss13 > 0) {
            ...
            $imposto13 = round($resIr13['desconto'], 2);
            if ($imposto13 > 0) {
                $items[] = ['categoria' => 'Descontos', 'nome' => 'IRRF (sobre 13º Salário)', 'referencia' => '-', 'valor' => $imposto13, 'isento' => false];
                $totalDescontos += $imposto13;
            }
        }
